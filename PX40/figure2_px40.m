...
% % % % % % indx_GS = find(lon_vel>=153.1 & lon_vel<=158.5);  %GS latitude range
% % % % % % 
% % % % % % for i = 1:nt
...
% % % % % %     k1 = find(vel0(1:indx0-1)>0,1,'last'); %easter boundary based on velocity
% % % % % %     if isempty(k1)
% % % % % %         k1 = find(transp(1:indx0-1)>0,1,'last'); %easter boundary based on transport
% % % % % %     end
...
% compute the area of each grid cell
 delt_z = diff(depth_ref(1:2));
 Area_GS = nan(ny_ref-1,nz_ref,nt);
 vel_alt = nan(length(lon_vel),nt);
...
for i = 1:nt
    transp = sum(vel_ref(:,:,i).*Area_GS(:,:,i),2,'omitnan');
    vel0 = mean(vel_ref(:,1:25,i),2); % use top 50-m averaged velocity to determine GS core
    [~,j] = max(vel0(indx_GS)); % based on near surface max velocity
    indx0 = indx_GS(j);
    GSspd(i) = vel_ref(indx0,1,i); % Surface max speed
    GSlat(i) = lat_vel(indx0,i);
    GSlon(i) = lon_vel(indx0,i);
    % find GS eastern and western boundary (where current changes sign)
    % or could be east west
    ...
    GStransp(i) = sum(transp(k1+1:k2-1));
    GStransp_z(:,i) = sum(vel_ref(k1+1:k2-1,:,i).*Area_GS(k1+1:k2-1,:,i)/delt_z,1,'omitnan');
    GSspd_z(:,i) = max(vel_ref(k1+1:k2-1,:,i),[],1);
    
     GSlon_EB(i) = lon_ref(k1+1);
     GSlon_WB(i) = lon_ref(k2);
     %   from altimeter SSH
        if ~isnan(mean(vel_alt(indx_GS,i),'omitnan'))
            [GSspd_alt(i),j] = min(vel_alt(indx_GS,i));
            GSlat_alt(i)=lat_vel(indx_GS(j));
            GStransp_alt(i) = ssh_ref_px40(k1+1,i)-ssh_ref_px40(k2-1,i);
        end
...
plot(time_avg,GSspd_alt,'color','r','LineWidth',1);hold on; 
...
set(h2,'ylim',[-1.6 -0.2],'xlim',[time_avg(1)-30 time_avg(end)+30])
...
set(h1,'ylim',[5 55],'xlim',[time_avg(1)-30 time_avg(end)+30])
...
text(double(time_avg(1)+90),-0.3,sprintf('R = %4.2f',C))

print -dpng -r600 results/figure2.PX40_GS.png
