...
for it = 1:nt
    ...
    lon_vel =  mid_lon(1,:)';
    vel_ref(:,:,it) = vel0;                                                %CHANGE VELOCITY DIRECTION
...
for i = 1:nt
    ...
    vel0 = mean(vel_ref(:,:,i),1,'omitmissing'); % use top 50-m averaged velocity to determine GS core
    ...
    % GStransp(i) = sum(transp);
    % GStransp_z(:,i) = sum(vel_ref(:,:,i).*Area_GS(:,:,i)/delt_z,2,'omitmissing');
    % GSspd_z(:,i) = max(vel_ref(:,:,i),[],2);
end
...
for i = 1:12
    ...
    mGSlon(i,1) = mean(GSlon2(ki),'omitmissing');
    mGSlon(i,2) = std(GSlon2(ki),'omitmissing');
    mGStransp(i,1) = mean(GStransp2(ki),'omitmissing');
    mGStransp(i,2) = std(GStransp2(ki),'omitmissing');
 end
...
ax=m_contfbar(0.20,[.57 .94],CS,CH,'axfrac',.025);
...
tmp(~isnan(tmp)) = 26;
tmp(isnan(tmp)) = -5;
% contourf(lon_ref,-depth_ref,tmp,-6:32:26); hold on
% colormap(gca,clr_T);
...
%contourf(lon_ref,-depth_ref,tmp,22:15:37); hold on
%colormap(gca,clr_S);
...
%contourf(lon_vel,-depth_ref,tmp,-1:1.4:1); hold on
colormap(gca,clr_V2);
[CS,CH] = contourf(lon_vel,-depth_ref,mvel,-0.85:0.05:0.85,'LineStyle','none');
...
xlim([153.1800 160])
tick_lon = 154:2:160;
label_lon = strcat(num2str(tick_lon'),'\circN');
