...
temp_ref=permute(temp_ref1(1:end-1,:,:),[3 2 1]);
salt_ref1  =ncread('/data2/Movar_analises/arquivos_pro_sal/NETCDF/AX97_MOVAR.nc','SA');
salt_ref=permute(salt_ref1(1:end-1,:,:),[3 2 1]);
vel_ref1  =ncread('/data2/Movar_analises/arquivos_pro_sal/NETCDF/AX97_MOVAR.nc','v_geo');
vel_ref=permute(vel_ref1(1:end-1,:,:),[3 2 1]);
...
% compute the area of each grid cell
 ...
 vel_alt = nan(length(lon_ref(1:end-1)),nt);
     ...
     Area_GS(:,:) = repmat(delt_y',[nz_ref 1])*delt_z;
...
indx_GS = find(lon_ref>=BLim(1) & lon_ref<=BLim(2));  %GS latitude range
for i = 1:nt
    transp = sum(vel_ref(1:51,1:end-1,i).*Area_GS(1:51,:),1,'omitnan');
    vel0 = mean(vel_ref(1:6,:,i),1); % use top 50-m averaged velocity to determine GS core
    [~,j] = min(vel0(indx_GS)); % based on near surface max velocity
    indx0 = indx_GS(j);
    GSspd(i) = vel_ref(1,indx0,i);
    GSlat(i) = lat_ref(indx0);
    GSlon(i) = lon_ref(indx0);
    
    % find GS northern and southern boundary (where current changes sign)
    k1 = find(transp(1:indx0-1)>0,1,'first'); %southern boundary based on transport
    if isempty(k1)
        k1 = find(vel0(1:indx0-1)>0,1,'first'); %southern boundary based on velocity
    end
...
plot(diaCB(1:end-1),GStransp/1e6,'color',[0 .45 .75],'LineWidth',1);
hold on
plot(diaCB(1:end-1),GStransp/1e6,'.','color',[0 .45 .75],'markersize',6);
ylabel('Transport (Sv)')
%set(gca,'position',[0.53 0.08 0.4 0.23])
set(gca,'position',[0.1 0.1 0.8 0.8])
h1=gca;h2=axes('position',get(h1,'position'));
plot(diaCB(1:end-1),GStransp_alt,'color','r','LineWidth',1);hold on
...
text(diaCB(1),0.12,'(h) GS transport')
text(diaCB(1),0.08,sprintf('R = %4.2f',C))

% print -dpng -r600 /data2/Movar_analises/nc_final/Figures_Shenfu/ax97/figure2.AX97_GS.png
