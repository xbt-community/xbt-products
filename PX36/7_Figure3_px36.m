...
%% Creazion cbar personalizzata simmetrica con limiti diversi cosi 0=bianco
...
% Normalizza i valori su una scala simmetrica attorno a zero
max_abs = max(abs([vmin, vmax]));

% Ricampiona la colormap su scala simmetrica
x = linspace(-max_abs, max_abs, nColors);  % scala simmetrica
x_actual = linspace(vmin, vmax, nColors);  % scala reale

% Interpola la colormap sulla scala reale
