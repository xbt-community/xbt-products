...
for i = 1:length(npf)
    ...
    % Plot only points passing the filter
    m_plot(lon_i(idx), lat_i(idx), '.r', 'markersize', 4);
end

m_plot(mean(lon_ref,2,'omitnan'),lat_ref,'k','linewidth',1)
m_grid('tickdir','out','color',xyclr,'xtick', 3, ...          % every 5 longitude degrees
    'ytick', 10, ...
    'fontsize', 8);
...
caxis([-3 12]) % Define Corlorbar between tra -2 e 12
lat_all = vertcat(lat_xbt{4});
lat_all = lat_all(lat_all >= -72 & lat_all <= -46);
hold on
plot(lat_all,0,'.k')% Plot a ping for alla XBT latitude casts of the first cruise
...
% Set ticks with NEGATIVE values (consistent with lat_ref)
xticks(-72:2:-46)
% Labels with step size 2 (only even values, without the minus sign)
vals = -72:2:-46;
...
% Invert the X-axis to display latitude in ascending order (from south to north)
